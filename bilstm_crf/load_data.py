...
vocab_file = "/data/bilstm_crf/char_vocabs.txt"
...
def data_generator(corpus_path, char2idx, label2idx, batch_size, shuffle=False):
    ...
    for line in lines:
        ...
        while True:
            ...
            pad_labels = [x + ["0"] * (max_len - len(x)) for x in buffer_labels]

            pad_datas = torch.tensor(
                [
                    [char2idx[w] if w in char2idx else char2idx["<UNK>"] for w in seq]
                    for seq in pad_datas
                ],
                dtype=torch.long,
            )
            pad_labels = torch.tensor(
                [[label2idx[t] for t in tag] for tag in pad_labels], dtype=torch.long
            )
