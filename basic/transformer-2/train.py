...
for epoch in range(N_EPOCHS):
    ...
    for batch in pbar:
        ...
        model.zero_grad()
        output, _ = model(src, trg[:, :-1]) # trg: [batch_size, trg_len] output: [batch_size, trg_len-1, output_dim]

        output_dim = output.shape[-1]
        output = output.contiguous().view(-1, output_dim) # [(trg_len - 1) * batch_size, output_dim]
        trg = trg[:, 1:].contiguous().view(-1) # [(trg_len - 1) * batch_size]
