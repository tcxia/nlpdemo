# -*- coding: utf-8 -*-
'''
# Created on 2021/01/15 12:46:55
...
def Num2Bit(Num, B):
    ...
    def integer2bit(integer, num_bits=B * 2):
        dtype = integer.type()
        exponent_bits = -torch.arange(-(num_bits - 1), 1).type(dtype).to(integer.device)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.contiguous().view(
            -1, int(self.feedback_bits /
                    self.num_quan_bits))  #需使用contiguous().view(),或者可修改为reshape
        out = self.sig(self.fc(out))
        out = out.contiguous().view(-1, 2, 24,
                                    16)  #需使用contiguous().view(),或者可修改为reshape
        for i in range(3):
            residual = out
